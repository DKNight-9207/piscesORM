...
class AsyncBaseSession(ABC):
    ...
    @abstractmethod
    async def get_all(self, table: Type[Table], *filters, load_relationships = True, read_only=False) -> List[Table]: ...

    @abstractmethod
    async def get_first(self, table: Type[Table], *filters, load_relationships = True, read_only=False) -> Table: ...
